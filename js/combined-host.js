...
let matching=null;
let demoRunId=0;
let demoLuckyState={luckyAward:null};
...
function phase4EnsureCombinedUnit(stage,index,startAt,endAt){const all=phase4CombinedHistory();if(!all)return null;const key=phase4UnitKey(stage,index);all[key]||={stage:Number(stage),index:Number(index),startAt:Number(startAt)||0,endAt:Number(endAt)||0,results:{},players:{}};if(startAt)all[key].startAt=Number(startAt);if(endAt)all[key].endAt=Number(endAt);return all[key];}
function phase4CombinedTimingOk(hist,at){const tap=Number(at);if(!hist||!Number.isFinite(tap))return false;if(tap<hist.startAt-1500||tap>hist.endAt+PHASE4_TAP_TOLERANCE_MS)return false;return now()<=hist.endAt+PHASE4_RECEIPT_WINDOW_MS;}
function phase4IsCurrent(stage,index){return liveRoom?.status==='playing'&&Number(liveRoom.stageIndex)===Number(stage)&&Number(liveRoom.unitIndex)===Number(index);}
function phase4IsSameUnit(stage,index){return ['playing','result'].includes(liveRoom?.status)&&Number(liveRoom.stageIndex)===Number(stage)&&Number(liveRoom.unitIndex)===Number(index);}
function phase4RecalcStage(uid,stageKey){const p=liveRoom?.players?.[uid];if(!p)return;let total=0;for(const h of Object.values(liveRoom._phase4History||{})){if(STAGE_META[h.stage]?.key===stageKey){if(stageKey==='matching')total+=Number(h.players?.[uid]?.score)||0;else total+=Number(h.results?.[uid]?.points)||0;}}p.scores[stageKey]=clamp(Math.round(total),0,1000);recalcLivePlayer(p);}
function phase4CombinedMatchPlayer(hist,uid){hist.players[uid]||={matchedPairIds:[],combo:0,mistakes:0,finishedAt:0,score:0};return hist.players[uid];}
function phase4RefreshCombinedLate(){void persistLive();if(liveRoom?.status==='finished')renderLiveFinal();else{renderLiveRank();updateLiveStageLabels();sendReplicaState();}}
...
function resetDemoPlayers(){DEMO_PLAYERS.forEach(p=>{p.scores={word:0,matching:0,sentence:0};});demoLuckyState={luckyAward:null};}
...
function startLiveUnit(stageIndex,unitIndex){if(!liveRoom||liveRoom.stageIndex!==stageIndex)return;clearLiveGuard();liveRoom.status='playing';liveRoom.unitIndex=unitIndex;liveRoom.unitResults={};liveRoom.unitStartAt=now()+150;let duration=10000;if(stageIndex===0){const q=liveRoom.quiz.questions[unitIndex];liveRoom.currentWordQuestion=q;liveRoom.currentMatchingRound=null;liveRoom.currentSentenceQuestion=null;duration=liveRoom.config.wordTime*1000;}else if(stageIndex===1){const r=liveRoom.matching.rounds[unitIndex];liveRoom.currentMatchingRound=r;liveRoom.currentWordQuestion=null;liveRoom.currentSentenceQuestion=null;duration=liveRoom.config.matchTime*1000;}else{const q=liveRoom.sentenceSet[unitIndex];liveRoom.currentSentenceQuestion={...q,shuffledTokens:phase3ShuffleTokens(q.tokens,`${liveRoom.pin}:${q.id}:${unitIndex}`)};liveRoom.currentWordQuestion=null;liveRoom.currentMatchingRound=null;duration=liveRoom.config.sentenceTime*1000;}liveRoom.unitEndAt=liveRoom.unitStartAt+duration;phase4EnsureCombinedUnit(stageIndex,unitIndex,liveRoom.unitStartAt,liveRoom.unitEndAt);liveRoom.resultEndAt=0;liveRoom.blindActive=(liveRoom.completedSteps/liveRoom.totalSteps)>=liveRoom.config.blindAt;if(liveRoom.blindActive&&!blindAudioPlayed){blindAudioPlayed=true;audio.playBlindTransition();}stageBgm(stageIndex,liveRoom.blindActive);if(stageIndex===2)sentenceAudio.sfx('start');else audio.playQuestionStart();lastTimerTickKey='';persistLive();renderLive();scheduleLive(()=>endLiveUnit('time'),duration+500);}
function handleWordAnswer(uid,p,receivedAt){if(!liveRoom||!liveRoom.players[uid])return;const index=Number(p.unitIndex),hist=phase4EnsureCombinedUnit(0,index,phase4IsCurrent(0,index)?liveRoom.unitStartAt:0,phase4IsCurrent(0,index)?liveRoom.unitEndAt:0);if(!hist?.startAt||!hist?.endAt||hist.results[uid])return;const tapAt=Number(receivedAt)||now();if(!phase4CombinedTimingOk(hist,tapAt))return;const q=liveRoom.quiz?.questions?.[index];if(!q)return;const choice=Number(p.choice),correct=choice===q.correctIndex,elapsed=Math.max(0,tapAt-hist.startAt),raw=calculateScore(liveRoom.config.wordTime*1000,elapsed,correct),points=Math.round(raw/liveRoom.config.wordCount),isCurrent=phase4IsCurrent(0,index),sameUnit=phase4IsSameUnit(0,index);hist.results[uid]={correct,points,choice,at:tapAt,recovered:!isCurrent};phase4RecalcStage(uid,'word');if(sameUnit)liveRoom.unitResults[uid]=hist.results[uid];if(isCurrent){if(correct){audio.playGift(points);const pl=liveRoom.players[uid];sendReplicaEvent('word',{kind:'correct',uid,name:pl.name,avatar:pl.avatar,points});}void persistLive();renderLive();if(allLiveResponded())setTimeout(()=>endLiveUnit('all'),220);}else phase4RefreshCombinedLate();}
function handleMatchPair(uid,p,receivedAt){if(!liveRoom||!liveRoom.players[uid])return;const index=Number(p.unitIndex),isCurrent=phase4IsCurrent(1,index),sameUnit=phase4IsSameUnit(1,index),hist=phase4EnsureCombinedUnit(1,index,isCurrent?liveRoom.unitStartAt:0,isCurrent?liveRoom.unitEndAt:0);if(!hist?.startAt||!hist?.endAt)return;const tapAt=Number(receivedAt)||now();if(!phase4CombinedTimingOk(hist,tapAt))return;const round=liveRoom.matching?.rounds?.[index],pairId=String(p.pairId||'');if(!round?.pairs?.some(x=>String(x.pairId)===pairId))return;const hp=phase4CombinedMatchPlayer(hist,uid);if(hp.matchedPairIds.includes(pairId))return;hp.matchedPairIds.push(pairId);hp.combo=(Number(hp.combo)||0)+1;const duration=liveRoom.config.matchTime*1000,elapsed=Math.max(0,tapAt-hist.startAt),remaining=clamp(1-elapsed/duration,0,1),perRound=1000/liveRoom.config.matchRounds,perPair=perRound/round.pairs.length,ratio=clamp(.55+.45*remaining+.025*Math.max(0,hp.combo-1),.50,1);let points=Math.round(perPair*ratio);if(hp.matchedPairIds.length>=round.pairs.length&&!hp.finishedAt)hp.finishedAt=tapAt;hp.score+=points;phase4RecalcStage(uid,'matching');const player=liveRoom.players[uid];if(sameUnit){player.matchedPairIds=[...hp.matchedPairIds];player.matchingCombo=hp.combo;player.matchingMistakes=hp.mistakes;if(hp.matchedPairIds.length>=round.pairs.length&&!liveRoom.unitResults[uid])liveRoom.unitResults[uid]={correct:true,points:Math.max(0,Math.round(hp.score)),complete:true,mistakes:hp.mistakes};}if(isCurrent){audio.playGift(points);void persistLive();renderLive();if(allLiveResponded())setTimeout(()=>endLiveUnit('all'),260);}else phase4RefreshCombinedLate();}
function handleMatchMistake(uid,p,receivedAt){if(!liveRoom||!liveRoom.players[uid])return;const index=Number(p.unitIndex),isCurrent=phase4IsCurrent(1,index),sameUnit=phase4IsSameUnit(1,index),hist=phase4EnsureCombinedUnit(1,index,isCurrent?liveRoom.unitStartAt:0,isCurrent?liveRoom.unitEndAt:0);if(!hist?.startAt||!hist?.endAt)return;const tapAt=Number(receivedAt)||now();if(!phase4CombinedTimingOk(hist,tapAt))return;const hp=phase4CombinedMatchPlayer(hist,uid);hp.mistakes=(Number(hp.mistakes)||0)+1;hp.combo=0;if(sameUnit){liveRoom.players[uid].matchingMistakes=hp.mistakes;liveRoom.players[uid].matchingCombo=0;}if(isCurrent){void persistLive();renderLive();}else phase4RefreshCombinedLate();}
function handleSentenceSubmit(uid,p,receivedAt){if(!liveRoom||!liveRoom.players[uid])return;const index=Number(p.unitIndex),isCurrent=phase4IsCurrent(2,index),hist=phase4EnsureCombinedUnit(2,index,isCurrent?liveRoom.unitStartAt:0,isCurrent?liveRoom.unitEndAt:0);if(!hist?.startAt||!hist?.endAt||hist.results[uid])return;const tapAt=Number(receivedAt)||now();if(!phase4CombinedTimingOk(hist,tapAt))return;const q=liveRoom.sentenceSet?.[index],order=Array.isArray(p.order)?p.order.map(String):[];if(!q)return;const correct=isCorrectSentenceOrder(order,q),elapsed=Math.max(0,tapAt-hist.startAt),raw=calculateScore(liveRoom.config.sentenceTime*1000,elapsed,correct),points=Math.round(raw/liveRoom.config.sentenceCount),sameUnit=phase4IsSameUnit(2,index);hist.results[uid]={correct,points,at:tapAt,recovered:!isCurrent};phase4RecalcStage(uid,'sentence');if(sameUnit)liveRoom.unitResults[uid]=hist.results[uid];if(isCurrent){if(correct){sentenceAudio.sfx('answer');const pl=liveRoom.players[uid];sendReplicaEvent('sentence',{kind:'correct',uid,name:pl.name,avatar:pl.avatar,points});}else sentenceAudio.sfx('submit');void persistLive();renderLive();if(allLiveResponded())setTimeout(()=>endLiveUnit('all'),240);}else phase4RefreshCombinedLate();}
...
function finishLive(){if(!liveRoom)return;clearLiveGuard();sentenceAudio.stopTension();audio.stopBgm();liveRoom.status='finished';liveRoom.finishedAt=now();liveRoom.blindActive=false;ensureLuckyAward(liveRoom,sortedLive(),liveRoom.finishedAt);liveRoom.currentWordQuestion=null;liveRoom.currentMatchingRound=null;liveRoom.currentSentenceQuestion=null;persistLive();renderLiveFinal();}

function renderLive(){if(!liveRoom)return;ensureReplicaStage(clamp(Number(liveRoom.stageIndex),0,2));sendReplicaState();}
function renderLiveFinal(){finalTone();setView('final');const ranked=sortedLive(),top=ranked.slice(0,3),cls=['first','second','third'];const lucky=ensureLuckyAward(liveRoom,ranked,liveRoom.finishedAt||now());$('podium').innerHTML=top.map((p,i)=>`<div class="podium-card ${cls[i]}"><div class="place">${i+1}위</div><div class="avatar">${esc(p.avatar)}</div><div class="name">${esc(p.name)}</div><div class="score">${liveTotal(p).toLocaleString()} / 3,000</div></div>`).join('');$('finalRanking').innerHTML=ranked.map((p,i)=>`<div class="final-row"><span>${i+1}</span><span>${esc(p.avatar)}</span><b>${esc(p.name)}</b><span class="sub">어휘 ${Math.round(p.scores.word)}</span><span class="sub">매칭 ${Math.round(p.scores.matching)}</span><span class="sub">문장 ${Math.round(p.scores.sentence)}</span><span class="total">${liveTotal(p).toLocaleString()}</span></div>`).join('');renderLuckyAward({anchor:$('podium'),award:lucky.award,eligible:lucky.eligible,onDraw:()=>audio.playLuckyDraw(),onTick:(progress)=>audio.playLuckyTick(progress),onReveal:()=>audio.playLuckyWinner()});if(lucky.changed)void persistLive();}
