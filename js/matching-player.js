...
async function join(){
  ...
  try{
    ...
    if(initial.status==='finished'&&!initial.players?.[uid])throw new Error('이미 종료된 게임입니다.'); const lateJoin=initial.status!=='lobby'; joined=true;joinConfirmed=false;localStorage.setItem(`kmp_name_${pin}`,name);localStorage.setItem(`kmp_avatar_${pin}`,selectedAvatar);$('myAvatar').textContent=selectedAvatar;$('waitingName').textContent=lateJoin?`${name}님, 진행 중인 게임에 입장 중...`:`${name}님, 입장 요청 중...`;show('waitingView'); await bus.send('join',{uid,name,avatar:selectedAvatar}); if(bus.mode==='local')bus.send('request-state',{});
