...
let bus=null, state=null, joined=false, submittedFor=-1, timerLoop=null, solo=null, soloScore=0;
...
const params=new URLSearchParams(location.search); const pinParam=params.get('pin'); const soloMode=params.get('solo')==='1';
...
function join(){
  ...
  const room=readRoom(pin); if(!room)return setError('방을 찾을 수 없습니다. 교사 화면에서 방을 먼저 만들어 주세요.');
  if(room.status!=='lobby')return setError('이미 게임이 시작된 방입니다.');
  bus?.close(); bus=new LocalBus(pin); bus.on(handleMessage); joined=true;
  localStorage.setItem(`kwb_name_${pin}`,name); localStorage.setItem(`kwb_avatar_${pin}`,selectedAvatar);
  bus.send('join',{uid,name,avatar:selectedAvatar}); bus.send('request-state',{});
  $('myAvatar').textContent=selectedAvatar; $('waitingName').textContent=`${name}님, 입장 완료!`; show('waitingView');
  setTimeout(()=>{const latest=readRoom(pin);if(latest){state=toPublic(latest);renderState();}},100);
}
function setError(t){$('joinError').textContent=t;}
function readRoom(pin){try{return JSON.parse(localStorage.getItem(`kwb_room_${pin}`)||'null')}catch{return null}}
...
function renderCountdown(){if(!state)return;const n=Math.max(1,Math.ceil((state.countdownEndAt-Date.now())/1000));$('playerCountdown').textContent=n;}
...
function startTimerLoop(){if(timerLoop)return;timerLoop=setInterval(()=>{if(!state)return;if(state.status==='countdown')renderCountdown();if(state.status==='playing'){const d=state.config.timeLimit*1000;const r=Math.max(0,Math.min(d,state.questionEndAt-Date.now()));$('playerTimerBar').style.width=`${r/d*100}%`;$('playerTimerText').textContent=(r/1000).toFixed(1);}},80)}
...
function startSoloMode(){
  ...
  uid='solo';selectedAvatar='🐻';soloScore=0;show('countdownView');let countEnd=Date.now()+3200;state={status:'countdown',countdownEndAt:countEnd};startTimerLoop();
  const wait=setInterval(()=>{renderCountdown();if(Date.now()>=countEnd){clearInterval(wait);runSoloQuestion(0);}},80);
}
function runSoloQuestion(index){
  ...
  state={status:'playing',questionIndex:index,questionTotal:solo.quiz.questions.length,currentQuestion:{direction:q.direction,prompt:q.prompt,options:q.options},questionStartAt:Date.now()+200,questionEndAt:Date.now()+200+duration,config:solo.config,players:{solo:{uid:'solo',name:'교사 테스트',avatar:'🐻',score:soloScore}},answeredUids:[]};
  show('quizView');renderQuiz(state.players.solo);startTimerLoop();
  const checker=setInterval(()=>{if(state.status==='playing'&&Date.now()>=state.questionEndAt){clearInterval(checker);soloReveal(index,null,0,false);}},80);
  $('answerGrid').onclick=e=>{const b=e.target.closest('.answer-btn');if(!b||state.status!=='playing')return;const choice=Number(b.dataset.i);const elapsed=Math.max(0,Date.now()-state.questionStartAt);const correct=choice===q.correctIndex;const pts=calculateScore(duration,elapsed,correct);clearInterval(checker);soloScore+=pts;soloReveal(index,q.answer,pts,correct);};
}
