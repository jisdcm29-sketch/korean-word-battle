...
function render(){
  ...
  const gameName=selectedGame==='sentence'?'문장 배틀':selectedGame==='matching'?'카드 매칭':selectedGame==='memory'?'기억력 배틀':selectedGame==='combined'?'종합 배틀':'어휘 배틀';
  els.guide.textContent=selectedGame==='sentence'?'문장 배틀에서 사용할 서울대 한국어 교재와 과를 선택하세요.':selectedGame==='matching'?'카드 매칭에 사용할 한국어·몽골어 어휘 자료를 선택하세요.':selectedGame==='memory'?'기억력 배틀에 사용할 한국어·몽골어 어휘 자료를 선택하세요.':selectedGame==='combined'?'세 게임을 연속 진행할 서울대 한국어 교재와 과를 선택하세요.':'어휘 배틀에서 사용할 자료를 선택하세요.';
  ...
  const note=selectedGame==='sentence'?' <span class="sample-warn">선택한 과의 어휘·문법·예문을 바탕으로 만든 문장 배틀 데이터가 자동으로 연결됩니다.</span>':selectedGame==='matching'?' <span class="sample-warn">한국어 카드와 몽골어 카드가 한 쌍으로 자동 구성됩니다.</span>':selectedGame==='memory'?' <span class="sample-warn">같은 뜻의 한국어·몽골어 카드가 뒤집힌 상태로 배치되어 기억력 대결을 진행합니다.</span>':selectedGame==='combined'?' <span class="sample-warn">공통 어휘 수정과 문장 추가 정답을 불러와 어휘 → 카드 매칭 → 문장 순서로 진행합니다.</span>':'';
...
function launch(){
  ...
  if(selectedGame==='word'){
    ...
    location.href=`memory-pairs.html?${params.toString()}`;
